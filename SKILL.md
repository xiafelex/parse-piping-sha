...
- A process-note row can be a two-line SHA cell rather than a multi-note group: `INSUL:`/`CLASS:`/`TRACE:` has a directly adjacent `graphic_ref + 1` code record such as `HI50`. When both direct Sheet anchors lie inside the same small closed `18/32` rectangle, bind both text records to that frame and render at their Sheet baselines using their PSM glyph size/width. This is a local reference-plus-anchor relation; do not infer it from visual proximity alone or apply it when the next reference is not a plausible code token.
