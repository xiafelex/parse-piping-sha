...
7. 页范围不是 `I### → handle`。只有已验证的 branch-body—pipe 端点直接接触才可在范围内
   写 `medium_anchor_candidate`；其余保持 `unresolved`，等待从锚点按构件顺序、转向和端点
   签名做全局传播。
8. 最终得到 `I### → source page + DXF handle(s)` 后，才逐页调用
...
`150` 是单点支架坐标（其第二端常为零坐标），不是可被图拓扑收缩的普通双端构件。完整
IDF 拓扑必须保留它；在边界复核图中以编号蓝色 `×` 绘出，并与 DXF 的支架切分共同作为
`100` 页归属与局部对应的锚点。重复的同坐标 `150` 仅显示一次，但原始行号/记录仍须保留。

先用 `score_support_150_page_covers_v1.py` 对每个候选页范围比较“该范围内的唯一 `150`
坐标数”与 DXF 的支架数。该结果只能淘汰或排序页范围：当它与独立的几何/分支框架证据
冲突，或待定 `100` 自身不接触 `150` 时，状态仍为 `unresolved`，必须输出人工截图，不能
因为支架总数相同而提升为匹配。

对页边界附近的 `100`，还必须保留并比较 `41` 的真实三维向量。不要把轴测图的屏幕上下
当作 Z 方向：应显式读取 `ΔZ`，并把连续两个 `41` 支管之间的短主管视为一个可辨识的
`branch–100–branch` 图案。该图案的支管数及 `ΔZ` 方向可打破仅用 `150` 仍并列的范围。
